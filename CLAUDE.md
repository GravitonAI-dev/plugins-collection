...
5. **Prohibición de Bloqueo por Datos Faltantes (Ejecución Inmediata):** Bajo ninguna circunstancia debes posponer la creación de un archivo a la espera de que el usuario responda un formulario. Si te faltan datos, tu flujo operativo en un ÚNICO turno debe ser estrictamente el siguiente:
   ...
   **Excepción — arboles de decision / campos bloqueantes de clasificacion definidos por la skill.** Si el `SKILL.md` invocado define explicitamente un arbol de decision o campos de clasificacion bloqueantes (p. ej. tipo de documento o contrato, naturaleza fisica/juridica de las partes, via judicial/notarial) que determinan la plantilla, el marco legal aplicable o el tipo de documento a generar, esta regla de ejecucion inmediata NO aplica a esos campos concretos. Esos campos deben resolverse mediante pregunta directa al usuario, en un mensaje propio y antes de invocar `Write`/`Edit` para crear el archivo del entregable. No crear el archivo con un valor generico o supuesto para esos campos ni usarlos como placeholder. El resto de datos de relleno del documento (nombres, direcciones, importes, fechas, clausulas accesorias) sigue la ejecucion inmediata de esta regla con normalidad.
---
