...
4. Fase 4: Edición Incremental Cláusula a Cláusula / Sección a Sección (Distinción estricta: datos de partes volcados directamente a disco con `search_clients`/`save_client` sustituyendo nombres en todo el documento [título H1, comparecencia y firmas] sin confirmación previa en chat vs cláusulas sustantivas con recogida en bloque `slot_filling_request`/diálogo -> Vista previa en chat -> "¿Confirmamos esta cláusula?" en chat -> `edit_file` + Hoja de Ruta con condicionales).
