...
3. Fase 3: Creación del Documento Base en Disco (Escritura `create_file` con Zero-Omission `{{DATO_FALTANTE}}`, volcado inmediato de datos conocidos de partes per `REG-CLI-04` + verificación prioritaria en `# WORKSPACE ACTIVE DOCUMENTS` + confirmación de ruta en chat encadenando de inmediato la Fase 4).
4. Fase 4: Edición Incremental Cláusula a Cláusula / Sección a Sección (Distinción estricta: datos de partes volcados directamente a disco con `search_clients`/`save_client` sin confirmación previa en chat vs cláusulas sustantivas con recogida en bloque `slot_filling_request`/diálogo -> Vista previa en chat -> "¿Confirmamos esta cláusula?" en chat -> `edit_file` + Hoja de Ruta con condicionales).
