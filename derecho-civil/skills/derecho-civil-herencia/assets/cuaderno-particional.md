# CUADERNO PARTICIONAL DE LA HERENCIA DE {{nombre_causante}}
...
En {{lugar}}, a {{fecha_particion}}, los herederos que a continuacion se relacionan, con capacidad legal para otorgar el presente documento de aceptacion y particion de la herencia causada por el fallecimiento de {{nombre_causante}}:
...
<!-- Si hay contador-partidor: Interviene {{nombre_contador_partidor}}, con NIF {{nif_contador_partidor}}, en calidad de contador-partidor designado {{origen_designacion}}, conforme al Art. 1057 del Codigo Civil. -->
...
<!-- Si testada: Que el causante fallecio bajo testamento {{tipo_testamento}} otorgado ante el Notario de {{plaza_notario}}, don/dona {{nombre_notario}}, el dia {{fecha_testamento}}, con numero de protocolo {{numero_protocolo}}, en el que {{disposiciones_testamentarias}}. -->
...
**A) ACTIVO.** Se relacionan los bienes y derechos que integran el caudal hereditario, con su valor a la fecha de la particion:
...
Total pasivo: {{total_pasivo}} euros ({{total_pasivo_letra}} euros).

## LIQUIDACION DEL HABER HEREDITARIO

Haber liquido partible = activo <!-- Si hay colacionables: + bienes colacionables --> - pasivo:

Activo {{total_activo}} + colacionables {{total_colacionables}} - pasivo {{total_pasivo}} = haber liquido {{haber_liquido}} euros ({{haber_liquido_letra}} euros).

**Calculo de cuotas y legitimas.**  
{{calculo_cuotas}}

## ADJUDICACIONES

En pago de sus respectivas cuotas, y guardando la posible igualdad conforme al Art. 1061 del Codigo Civil, se adjudican los bienes del modo siguiente:
...
Los comparecientes **aceptan la herencia** del causante {{modo_aceptacion}} y prestan su plena conformidad al presente cuaderno particional, a las operaciones de inventario, avaluo, liquidacion y adjudicacion que anteceden.
...
> **Advertencias:**  
> 1. Este documento es un DRAFT generado automaticamente. Debe ser revisado por un abogado y, si la herencia incluye inmuebles, elevado a escritura publica notarial para su inscripcion en el Registro de la Propiedad.  
> 2. Version del Codigo Civil verificada: {{fecha_verificacion_cc}}.  
> 3. Impuesto de Sucesiones y Donaciones: tributo AUTONOMICO. Plazo general de autoliquidacion de 6 meses desde el fallecimiento (prorrogable). Verificar las bonificaciones de la comunidad autonoma {{comunidad_autonoma}} antes de liquidar.  
...
> 6. La particion requiere el acuerdo de todos los herederos; a falta de acuerdo, procede la particion judicial o el nombramiento de contador-partidor.
