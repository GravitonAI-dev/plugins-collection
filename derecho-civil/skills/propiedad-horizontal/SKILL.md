...
1. **Datos de la comunidad** *(dato objetivo — búsqueda prioritaria con `search_clients`, recogida en lote con `slot_filling_request` si es nueva, volcado directo a disco per REG-CLI-04)*. Anuncio fijo: "Comenzamos por la identificacion de la comunidad de propietarios." Invoca `search_clients` prioritariamente. Si es nueva, solicita en bloque vía `slot_filling_request`: a) denominacion; b) NIF; c) direccion completa de la finca; d) nombre y NIF de quien ejerce las funciones de secretario o secretario-administrador, y su cargo exacto; e) nombre del presidente. Antes de cerrar el bloque, si el secretario es un administrador profesional, pregunta si va a intervenir profesionalmente en la reclamacion, porque de ello depende que sea o no necesario el visto bueno del presidente (articulo 21.3). Si es nueva cliente/entidad, resuelve `restricted_human_in_the_loop_request` (`REG-CLI-03`). Conforme a `REG-CLI-04`, vuelca de inmediato al documento con `edit_file` (o ya volcado en `create_file`) sin requerir confirmación en chat, sustituyendo exhaustivamente los nombres en todas sus apariciones a lo largo del documento (título/encabezado H1, comparecencia y firmas; use `replace_all: true` o ediciones que cubran las 3 ubicaciones).
2. **Datos del propietario deudor** *(dato objetivo — búsqueda prioritaria con `search_clients`, recogida en lote con `slot_filling_request` si es nuevo, volcado directo a disco per REG-CLI-04)*. Anuncio fijo: "Pasamos a la identificacion del propietario deudor." Invoca `search_clients` prioritariamente. Si es nuevo, solicita en bloque vía `slot_filling_request`: a) nombre o razon social; b) NIF o CIF si se conoce; c) elemento privativo (piso, letra, planta, plaza o local); d) cuota de participacion; e) domicilio designado por el deudor para las notificaciones de la comunidad. Si no ha designado ninguno, explicar que el requerimiento judicial se practicara en el propio piso o local (articulo 815.2 de la LEC) y hacerlo constar. Si es nuevo cliente, resuelve `restricted_human_in_the_loop_request` (`REG-CLI-03`). Vuelca directamente al documento con `edit_file` sin confirmación en chat per `REG-CLI-04`, sustituyendo exhaustivamente los nombres en todas sus apariciones (título H1, comparecencia y firmas).
...
1. **Datos del propietario demandante** *(dato objetivo — búsqueda prioritaria con `search_clients`, recogida en lote con `slot_filling_request` si es nuevo, volcado directo a disco per REG-CLI-04)*. Anuncio fijo: "Comenzamos por su identificacion como parte demandante." Invoca `search_clients` prioritariamente. Si es nuevo, solicita en bloque vía `slot_filling_request`: a) nombre completo o razon social; b) NIF o CIF; c) domicilio a efectos de notificaciones; d) elemento privativo y cuota de participacion; e) nombre del procurador y del letrado, preceptivos en el juicio ordinario. Si es nuevo cliente, resuelve `restricted_human_in_the_loop_request` (`REG-CLI-03`). Vuelca de inmediato a disco con `edit_file` (o ya volcado en `create_file`) sin confirmación en chat per `REG-CLI-04`.
2. **Datos de la comunidad demandada** *(dato objetivo — búsqueda prioritaria con `search_clients`, recogida en lote con `slot_filling_request` si es nueva, volcado directo a disco per REG-CLI-04)*. Anuncio fijo: "Pasamos a la identificacion de la comunidad demandada." Invoca `search_clients` prioritariamente. Si es nueva, solicita en bloque vía `slot_filling_request`: a) denominacion; b) NIF si se conoce; c) direccion de la finca; d) nombre del presidente, que es quien la representa en juicio. Si es nueva entidad cliente, resuelve `restricted_human_in_the_loop_request` (`REG-CLI-03`). Vuelca de inmediato a disco con `edit_file` sin confirmación en chat per `REG-CLI-04`.
...
1. **Datos de la comunidad y del presidente** *(dato objetivo — búsqueda prioritaria con `search_clients`, recogida en lote con `slot_filling_request` si es nueva, volcado directo a disco per REG-CLI-04)*. Anuncio fijo: "Comenzamos por la identificacion de la comunidad y de quien firma el requerimiento." Invoca `search_clients` prioritariamente. Si es nueva, solicita en bloque vía `slot_filling_request`: a) denominacion de la comunidad; b) NIF; c) direccion de la finca; d) nombre y NIF del presidente. Si es nueva entidad cliente, resuelve `restricted_human_in_the_loop_request` (`REG-CLI-03`). Vuelca de inmediato a disco con `edit_file` (o en `create_file`) sin confirmación en chat per `REG-CLI-04`.
2. **Datos del infractor** *(dato objetivo — búsqueda prioritaria con `search_clients`, recogida en lote con `slot_filling_request` si es nuevo, volcado directo a disco per REG-CLI-04)*. Anuncio fijo: "Pasamos a la identificacion de quien desarrolla la actividad." Invoca `search_clients` prioritariamente. Si es nuevo, solicita en bloque vía `slot_filling_request`: a) nombre o razon social; b) NIF o CIF si se conoce; c) si es propietario u ocupante del elemento privativo; d) elemento privativo; e) domicilio a efectos de notificaciones. Si es ocupante y no propietario, advierte de que conviene remitir copia al propietario, porque la eventual demanda debe dirigirse contra ambos (articulo 7.2). Si es nuevo cliente, resuelve `restricted_human_in_the_loop_request` (`REG-CLI-03`). Vuelca directamente a disco con `edit_file` sin confirmación en chat per `REG-CLI-04`.
