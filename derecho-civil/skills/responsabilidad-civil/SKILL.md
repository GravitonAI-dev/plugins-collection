...
1. **Datos del perjudicado** *(dato objetivo — búsqueda prioritaria con `search_clients`, recogida en lote con `slot_filling_request` si es nuevo, volcado directo a disco per REG-CLI-04)*. Anuncio fijo: "Comenzamos por sus datos identificativos." Invoca `search_clients` prioritariamente. Si es nuevo, solicita en bloque vía `slot_filling_request`: a) nombre y apellidos o razon social; b) DNI, NIE o CIF segun corresponda; c) domicilio a efectos de notificaciones; d) telefono y correo electronico de contacto; e) si interviene letrado, su nombre y numero de colegiado. Si es nuevo cliente, resuelve `restricted_human_in_the_loop_request` (`REG-CLI-03`). Conforme a `REG-CLI-04`, vuelca de inmediato a disco con `edit_file` (o ya volcado en `create_file`) sin confirmación en chat, sustituyendo exhaustivamente los nombres en todas sus apariciones a lo largo del documento (título/encabezado H1, comparecencia y firmas; use `replace_all: true` o ediciones que cubran las 3 ubicaciones).
...
3. **Datos del destinatario o destinatarios** *(dato objetivo — búsqueda prioritaria con `search_clients`, recogida en lote con `slot_filling_request` si son nuevos, volcado directo a disco per REG-CLI-04)*. Anuncio fijo: "Pasamos a los datos de la parte a la que se dirige la reclamacion." Invoca `search_clients` prioritariamente. Por cada destinatario nuevo, solicita en bloque vía `slot_filling_request`: a) nombre o razon social; b) documento de identidad o CIF; c) domicilio. Si es aseguradora, ademas: numero de poliza y referencia del siniestro, si se conocen. Si son nuevos clientes/entidades, resuelve `restricted_human_in_the_loop_request` (`REG-CLI-03`). Vuelca de inmediato a disco con `edit_file` sin turno de confirmación en chat per `REG-CLI-04`, sustituyendo exhaustivamente los nombres en todas sus apariciones (título H1, comparecencia y firmas). **Si V4 = no**, explica que se incluira el requerimiento del ultimo inciso del Art. 76 LCS para que el causante manifieste la existencia y el contenido de su seguro, y en circulacion menciona el fichero de vehiculos asegurados del Consorcio (Art. 2.2 TRLRCSCVM).
...
1. **Datos del perjudicado** *(dato objetivo — búsqueda prioritaria con `search_clients`, recogida en lote con `slot_filling_request` si es nuevo, volcado directo a disco per REG-CLI-04)*. Anuncio fijo: "Comenzamos por sus datos identificativos." Igual estructura que en la HOJA RECLAMACION vía `slot_filling_request`, volcando directamente a disco per `REG-CLI-04`.
2. **Datos de la aseguradora y del expediente** *(dato objetivo — búsqueda prioritaria con `search_clients`, recogida en lote con `slot_filling_request` si es nueva, volcado directo a disco per REG-CLI-04)*. Anuncio fijo: "Pasamos a los datos de la aseguradora y del expediente del siniestro." Invoca `search_clients` prioritariamente. Si es nueva, solicita en bloque vía `slot_filling_request`: a) denominacion y CIF de la aseguradora; b) domicilio al que se dirige el escrito; c) numero de poliza; d) referencia del siniestro o del expediente. Si es nueva entidad cliente, resuelve `restricted_human_in_the_loop_request` (`REG-CLI-03`). Vuelca de inmediato a disco con `edit_file` sin confirmación en chat per `REG-CLI-04`.
