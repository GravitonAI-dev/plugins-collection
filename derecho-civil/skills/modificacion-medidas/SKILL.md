...
- **No usar para fijar medidas por primera vez.** Si no hay sentencia, decreto o convenio anterior, corresponde a `divorcio` (medidas definitivas de la ruptura), no a esta skill.
...
- **No usar para reclamar pensiones impagadas.** Es ejecucion forzosa del Art. 776 LEC: derivar a `ejecucion-titulos` si existe en el marketplace y, si no existe, escalar via derivación formal. Un mismo caso puede necesitar ejecutar los atrasos y modificar hacia delante: son dos actuaciones distintas y no se mezclan en un solo escrito.
