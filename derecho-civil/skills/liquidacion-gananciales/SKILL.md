...
1. **Primer conyuge** *(dato objetivo — `slot_filling_request` con confirmación en el chat)*. Anuncio fijo: "Comenzamos por la identificacion de ambos conyuges." Solicita en bloque mediante `slot_filling_request`: a) nombre completo; b) NIF; c) domicilio. Al recibir el resultado de la tool, muestra en el chat vista previa conjunta con el segundo cónyuge (o individual), pide confirmación ("¿Confirmamos estos datos?") y aplica el `Edit`.
2. **Segundo conyuge** *(dato objetivo — `slot_filling_request` con confirmación en el chat)*. Anuncio fijo: "Identificado el primer conyuge, pasamos al segundo." Solicita en bloque mediante `slot_filling_request`: a) nombre completo; b) NIF; c) domicilio. Vista previa en chat, confirmación y `Edit`.
3. **Matrimonio y disolucion de la sociedad** *(dato objetivo con validacion — `slot_filling_request` con confirmación en el chat)*. Anuncio fijo: "Pasamos a los datos del matrimonio y a la disolucion de la sociedad de gananciales." Solicita en bloque mediante `slot_filling_request`: a) fecha y lugar de celebracion; b) registro civil de inscripcion; c) origen del regimen; d) fecha y causa de la disolucion (o de admision de la demanda). Aplica validaciones, muestra vista previa en chat, pide confirmación y aplica `Edit`.
4. **Activo: bienes gananciales** *(clausula de negociacion — explicar antes de decidir)*. Anuncio fijo: "Pasamos ahora al inventario del activo: los bienes y derechos comunes." Explica antes de pedir la relacion, con base en `references/cc-activo-pasivo-y-presuncion.md`. Despues solicita la relación de bienes y sus datos (descripcion, valor, datos registrales y referencia catastral en inmuebles). Vista previa en chat, confirmación y `Edit`.
...
6. **Pasivo: deudas pendientes a cargo de la sociedad** *(dato objetivo con validacion — `slot_filling_request` con confirmación en el chat)*. Anuncio fijo: "Pasamos al pasivo: las deudas pendientes a cargo de la sociedad." Solicita las deudas en bloque mediante `slot_filling_request`: concepto, acreedor y **saldo pendiente certificado a una fecha concreta**. Vista previa en chat, confirmación y `Edit`.
...
1. **Conyuge solicitante** *(dato objetivo — `slot_filling_request` con confirmación en el chat)*. Anuncio fijo: "Comenzamos por su identificacion como parte solicitante." Solicita en bloque mediante `slot_filling_request`: a) nombre completo; b) NIF; c) domicilio. Vista previa en chat, confirmación y `Edit`.
2. **Conyuge contrario** *(dato objetivo — `slot_filling_request` con confirmación en el chat)*. Anuncio fijo: "Pasamos a la identificacion del otro conyuge." Solicita en bloque mediante `slot_filling_request`: a) nombre completo; b) NIF; c) domicilio. Vista previa en chat, confirmación y `Edit`.
