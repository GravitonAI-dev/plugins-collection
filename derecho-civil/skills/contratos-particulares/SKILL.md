...
> - **Cláusulas Sustantivas / Negociables:** (Importe, intereses, plazo, mora, vencimiento, garantías, etc.). Siguen estrictamente el ciclo: Negociación/Datos -> Vista Previa en chat -> `¿Confirmamos esta cláusula?` -> `edit_file`.
...
- `[dato objetivo]`: se solicita mediante `slot_filling_request` si agrupa datos, o pregunta directa. Si refiere a datos sustantivos de la operación (bienes, deudas, importes), vista previa y confirmación obligatoria en el chat (`¿Confirmamos esta cláusula?`). Si refiere a personas físicas o jurídicas, rige `REG-CLI-04` (volcado directo sin confirmación en chat).
- `[negociacion]`: **explica primero el regimen legal por defecto y las consecuencias de cada opcion, y solo despues pide la decision en el chat.** Nunca registres el valor sin haber explicado. Muestra vista previa y confirma en el chat antes de escribirlo.

### Secciones — HOJA PRESTAMO

1. **Parte prestamista** *(`[dato objetivo]` — búsqueda prioritaria con `search_clients`; volcado directo a disco per REG-CLI-04)*. Anuncio fijo: "Comenzamos por la identificacion de quien entrega el dinero." Ejecute prioritariamente `search_clients`. Si la ficha cubre los datos, úselos directamente; si falta información, solicite solo los campos pendientes con `slot_filling_request`. Si es nuevo cliente, aplique `REG-CLI-03`. Vuelque directamente los datos al documento mediante `edit_file` (o en `create_file` si ya constaban), sustituyendo exhaustivamente los nombres en todas sus apariciones (título/encabezado H1, comparecencia y firmas; use `replace_all: true` o ediciones que cubran las 3 ubicaciones), informe al usuario en chat y continúe sin confirmación previa en chat.
2. **Parte prestataria** *(`[dato objetivo]` — búsqueda prioritaria con `search_clients`; volcado directo a disco per REG-CLI-04)*. Anuncio fijo: "Pasamos a la identificacion de quien recibe el prestamo." Mismo procedimiento: volcado directo a disco per `REG-CLI-04`, sustituyendo exhaustivamente los nombres en todas sus apariciones (título H1, comparecencia y firmas), informe en chat y continúe sin confirmación previa en chat.
...
1. **Parte acreedora** *(`[dato objetivo]` — búsqueda prioritaria con `search_clients`; volcado directo a disco per REG-CLI-04)*. Anuncio fijo: "Comenzamos por la identificacion de la parte acreedora." Volcado directo a disco per `REG-CLI-04`, informe en chat y continúe sin confirmación previa en chat.
2. **Parte deudora** *(`[dato objetivo]` — búsqueda prioritaria con `search_clients`; volcado directo a disco per REG-CLI-04)*. Anuncio fijo: "Pasamos a la identificacion de la parte deudora." Volcado directo a disco per `REG-CLI-04`, informe en chat y continúe sin confirmación previa en chat.
...
1. **Parte comodante** *(`[dato objetivo]` — búsqueda prioritaria con `search_clients`; volcado directo a disco per REG-CLI-04)*. Anuncio fijo: "Comenzamos por la identificacion de quien cede el uso de la cosa." Búsqueda prioritaria con `search_clients`, volcado directo a disco per `REG-CLI-04`, informe en chat y continúe sin confirmación previa en chat.
2. **Parte comodataria** *(`[dato objetivo]` — búsqueda prioritaria con `search_clients`; volcado directo a disco per REG-CLI-04)*. Anuncio fijo: "Pasamos a la identificacion de quien recibe la cosa en uso." Búsqueda prioritaria con `search_clients`, volcado directo a disco per `REG-CLI-04`, informe en chat y continúe sin confirmación previa en chat.
...
1. **Parte vendedora** *(`[dato objetivo]` — búsqueda prioritaria con `search_clients`; volcado directo a disco per REG-CLI-04)*. Anuncio fijo: "Comenzamos por la identificacion de la parte vendedora." Búsqueda prioritaria con `search_clients`, volcado directo a disco per `REG-CLI-04`, informe en chat y continúe sin confirmación previa en chat.
2. **Parte compradora** *(`[dato objetivo]` — búsqueda prioritaria con `search_clients`; volcado directo a disco per REG-CLI-04)*. Anuncio fijo: "Pasamos a la identificacion de la parte compradora." Búsqueda prioritaria con `search_clients`, volcado directo a disco per `REG-CLI-04`, informe en chat y continúe sin confirmación previa en chat.
