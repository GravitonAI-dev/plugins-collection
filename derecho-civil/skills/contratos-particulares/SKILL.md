...
1. **Parte prestamista** *(`[dato objetivo]` — búsqueda prioritaria con `search_clients`; volcado directo a disco per REG-CLI-04)*. Anuncio fijo: "Comenzamos por la identificacion de quien entrega el dinero." Ejecute prioritariamente `search_clients`. Si la ficha cubre los datos, úselos directamente; si falta información, solicite solo los campos pendientes con `slot_filling_request`. Si es nuevo cliente, aplique `REG-CLI-03`. Vuelque directamente los datos al documento mediante `edit_file` (o en `create_file` si ya constaban), informe al usuario en chat y continúe sin confirmación previa en chat.
2. **Parte prestataria** *(`[dato objetivo]` — búsqueda prioritaria con `search_clients`; volcado directo a disco per REG-CLI-04)*. Anuncio fijo: "Pasamos a la identificacion de quien recibe el prestamo." Mismo procedimiento: volcado directo a disco per `REG-CLI-04`, informe en chat y continúe sin confirmación previa en chat.
