...
2. **Representacion procesal (dato objetivo — `slot_filling_request` con confirmación en el chat).** Anuncio: "Corresponde identificar la representacion procesal." Solicita en bloque mediante `slot_filling_request`: (a) nombre del procurador; (b) nombre del letrado. Vista previa en chat, confirmación y `Edit`.
...
1. **Conyuge demandante (dato objetivo — `slot_filling_request` con confirmación en el chat).** Anuncio de apertura: "Procedemos a la identificacion de la parte demandante." Solicita en bloque mediante `slot_filling_request`: (a) nombre completo; (b) DNI o NIE; (c) domicilio. Vista previa en chat, confirmación y `Edit`.
2. **Conyuge demandado (dato objetivo — `slot_filling_request` con confirmación en el chat).** Anuncio: "Identificada la parte demandante, pasamos a la parte demandada." Solicita en bloque mediante `slot_filling_request`: (a) nombre completo; (b) DNI o NIE; (c) domicilio. Vista previa en chat, confirmación y `Edit`.
3. **Matrimonio (dato objetivo).** Anuncio: "Pasamos a los datos del matrimonio." Solicita mediante `slot_filling_request` o pregunta: (a) fecha y lugar de celebracion; (b) registro civil; (c) regimen economico (gananciales / separacion de bienes). Vista previa en chat, confirmación y `Edit`.
4. **Hijos (solo si V3 = Si; dato objetivo — `slot_filling_request` con confirmación en el chat).** Anuncio: "Corresponde identificar a los hijos comunes." Solicita en bloque mediante `slot_filling_request` nombre y fecha de nacimiento de cada hijo. Vista previa en chat, confirmación agrupada y `Edit`.
