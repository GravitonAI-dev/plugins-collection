...
> - **Naturaleza de plantilla de los archivos en workspace y regla de inversión de flujo:** A diferencia de las skills sustantivas de tramitación (donde el objetivo es rellenar los placeholders con datos reales del cliente), en `registrar-plantillas` el flujo es estrictamente **inverso**: abstraer datos concretos de casos particulares o redactar cláusulas modelo para crear o perfeccionar una plantilla reutilizable. Por consiguiente:
>   1. Los documentos del workspace gestionados o creados en esta skill son **borradores de plantilla (assets)**, no documentos de clientes.
>   2. La presencia de marcadores `{{VARIABLE}}` en el documento del workspace es **intencional, permanente y obligatoria**.
>   3. Queda **TERMINANTEMENTE PROHIBIDO** intentar rellenar o solicitar datos reales (vía formularios `slot_filling_request` o preguntas en el chat) para sustituir los marcadores `{{VARIABLE}}` de la plantilla.
>   4. Toda regla o directiva de "cero placeholders", "zero-omission de rellenado" o "eliminar placeholders sin resolver" propia de la redacción de escritos finales de clientes es **TOTAL Y EXPRESAMENTE INAPLICABLE** a esta skill.
...
```json
{
  "form_data": [
    {
      "id": "tipo_asistencia_plantilla",
      "rationale": "Permitir al usuario seleccionar mediante formulario interactivo el tipo de asistencia requerido sobre su plantilla preexistente antes de aplicar modificaciones o persistir cambios.",
      "question": "¿Qué tipo de asistencia necesitas para esta plantilla?",
      "options": [
        {
        ...
        },
        {
          ...
          "label": "Mejorar el contenido (optimizar redacción, estructura y cláusulas sugeridas)"
        },
        {
          "id": "actualizar_contenido",
          "label": "Actualizar contenido (incorporar cambios o ajustes directos)"
        }
      ]
    }
  ]
...
- **Si selecciona `convertir_placeholders` (Convertir datos a placeholders genéricos):**
  ...
  3. Si el archivo está en el workspace, aplica las sustituciones mediante `edit_file` para que el usuario pueda visualizar la plantilla parametrizada en tiempo real en el editor; si provino de texto en el chat, muestra el borrador parametrizado.
     - **Regla de ejecución ininterrumpida hasta completar:** Aplica de forma continua y autónoma TODAS las sustituciones necesarias hasta que la plantilla quede completamente parametrizada. **Queda TERMINANTEMENTE PROHIBIDO detenerte a mitad de la tarea para preguntar al usuario si continúas aplicando las sustituciones restantes**. Debes ejecutar el trabajo completo de principio a fin en este mismo turno.
     - **Regla de precisión en `edit_file`:** Selecciona como `old_string` fragmentos o líneas unívocas exactas copiadas del archivo. NUNCA introduzcas barras invertidas de escape manuales (ej. no pongas `\.`, `\+` ni `\\`), ya que el motor de emparejamiento gestiona los escapes automáticamente.
     - **Culminación de la tarea:** Una vez aplicadas todas las sustituciones en el editor, invoca `get_revision` para verificar el documento.
  4. Una vez completadas todas las sustituciones y verificado el documento, informa al usuario del resultado y pregúntale si desea realizar ajustes adicionales o proceder a persistir la plantilla en el sistema.
...
3. **Cobertura de Variables Obligatorias:** La plantilla DEBE contener los marcadores `{{VARIABLE}}` que corresponden a los inputs esenciales que la skill requiere y cumplimenta durante su ejecución (consultar los `inputs:` del `SKILL.md` de la skill destino: datos de las partes, objeto, importes, plazos, etc.). Se audita la **presencia** de estos marcadores en el texto de la plantilla; queda expresamente prohibido sustituirlos por datos concretos de clientes.
...
10. **Equivalencia de Vía y Cero Redundancia de Datos por Chat (REG-DAT-01):** Todo contenido, corrección, nombre o parámetro para la plantilla puede ser suministrado indistintamente mediante formularios interactivos o por texto libre en el chat. Si el usuario ya facilitó el texto, la descripción o el nombre directamente en su mensaje, ingiérelo de inmediato sin convocar formularios redundantes. Si canceló o cerró un formulario para plantear una duda o comentario, atiende su consulta en el chat y reanuda oportunamente el proceso de registro.
