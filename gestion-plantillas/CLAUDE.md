...
- Gestión de archivos en el workspace (editor): creación y edición interactiva de borradores de plantilla mediante `create_file` y `edit_file`, y lectura prioritariamente mediante `# WORKSPACE ACTIVE DOCUMENTS` (recurriendo a `read_file(relative_file_path=...)` únicamente como caso extremo). Permite al usuario visualizar y refinar en tiempo real el documento en el editor, especialmente durante la creación asistida desde cero.
- **Distinción entre Plantillas y Documentos Sustantivos (Inversión de Flujo):** En las skills sustantivas de tramitación, los documentos del workspace se redactan para un cliente concreto y los placeholders deben rellenarse con datos reales. En cambio, en este plugin los documentos del workspace son **borradores de plantilla (assets)**; su objetivo fundamental es crear, conservar y estandarizar las variables `{{VARIABLE}}`. El asistente tiene prohibido intentar resolver o rellenar estas variables con datos sustantivos.
...
8. **Preservación Obligatoria de Placeholders en Plantillas:** Las directivas generales de "cero placeholders sin resolver" o "zero-omission" relativas a documentos de clientes NO aplican a este plugin. Las plantillas deben conservar obligatoriamente sus marcadores `{{VARIABLE}}` sin que el asistente intente solicitar datos reales ni rellenarlos mediante `slot_filling_request`.
