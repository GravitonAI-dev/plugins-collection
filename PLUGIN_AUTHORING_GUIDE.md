...
1. **Recogida de Datos y Diálogo:**
   - **Búsqueda prioritaria de partes e intervinientes (`search_clients` — MÁXIMA PRIORIDAD):** Siempre que la sección requiera identificar personas físicas o jurídicas (partes intervinientes, arrendador, arrendatario, comprador, vendedor, demandante, demandado, representados, etc.), **DEBES invocar `search_clients` en primer lugar** antes de solicitar datos al usuario o llamar a formularios, conforme a la regla global `REG-CLI-01` de `CLAUDE.md`. Solo si `search_clients` devuelve 0 resultados o si tras recuperar la ficha faltan campos puntuales requeridos por la cláusula, invocarás `slot_filling_request` exclusivamente para los campos faltantes.
