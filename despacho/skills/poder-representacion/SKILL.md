...
* **Si `[V1 = poder_especial]` → Plantilla: `assets/template-minuta-poder-especial.md`.**
* **Precedencia:** `V1` fija siempre el instrumento; `V4` no elige plantilla, delimita las facultades dentro de la minuta elegida. **Si `[V1 = poder_general]` y `[V4 = especial_facultades_tasadas]`, la respuesta es contradictoria:** un poder general para pleitos no se otorga con facultades tasadas. No elijas por el cliente: explica la diferencia en el chat y pide que confirme cual de las dos quiere antes de continuar.
