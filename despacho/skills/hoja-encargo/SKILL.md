...
* **Si `[V1 = hoja_encargo]` y `[V2 = resultado]` → Plantilla: `assets/template-hoja-encargo-resultado.md`.** La cuota litis tiene su propia hoja: no se pacta sobre la hoja ordinaria.
* **Si `[V1 = hoja_encargo]` y `[V2 = fijo, por_horas, mixta o provision_periodica]` → Plantilla: `assets/template-hoja-encargo-profesional.md`.**
...
* **Si `[V1 = encargo_resultado]` → Plantilla: `assets/template-hoja-encargo-resultado.md`,** con cualquier valor de `V2`.
* **Precedencia:** `V1` fija siempre el documento. `V2 = resultado` solo cambia la hoja dentro de `V1 = hoja_encargo`; en `V1 = presupuesto` y `V1 = fin_encargo` no altera la plantilla, solo activa la advertencia de cuota litis. **Advertencia obligatoria en el chat:** la retribución vinculada al resultado exige una redacción especialmente cuidadosa —definición exacta del resultado que genera el derecho, tratamiento de los acuerdos transaccionales, de la renuncia del cliente y del desistimiento, y régimen de las costas percibidas—, y su admisibilidad y límites deben comprobarse en la **normativa deontológica vigente del colegio de adscripción**, que puede imponer condiciones. Verifícalo con `web_search` y adviértelo antes de redactar.
...
4. **Propuesta de plantilla oficial del sistema que ha resuelto el enrutamiento de la Fase 1.3.** Menciónala por su denominación formal (sin mostrar rutas internas ni volcar su contenido); si el enrutamiento asigno varios documentos, nombralos todos y en el orden en que se van a redactar. **No propongas una plantilla distinta de la enrutada.**
5. **Elección de Plantilla Base:** Aplica el protocolo universal de `REG-AST-01` (`CLAUDE.md`), convocando en ese mismo turno el formulario interactivo `restricted_human_in_the_loop_request` para que el usuario elija entre la plantilla del sistema o aportar su propia minuta.
